...
\ used. Most significant byte is sent first. 
\ ( 16data cmd -- nn) 
...
\ cmd0       GO_IDLE_STATE        Used to change from SD to SPI mode
\ ( -- f)
: cmd0
        0  cs                    
        0  0  cmd 
...
\ dbuff   :   512 byte sector read/write buffer
variable dbuff 510 allot

\ Fetch byte/word/dword from an offset into dbuff.
: d8  dbuff + c@ ;
: d16 dbuff + @ ;
: d32 dbuff + 2@ swap ;
...
\ Read 1 sector.
\ surprise: j1a is little endian, but so is sector read.
: readsec ( d -- u u )
	2dup sector 2!
	0 cs
    17 dcmd
...
\ read the first 512 byte sector in a cluster (of 8 sectors)
: readclus ( d -- )
  2dup cluster 2!
  rootClus 2@ d-
  d2* d2* d2*
